...
"""
Sudden left lane change scenario:

The scenario realizes a driving behavior on the highway.
The user-controlled ego vehicle is driving straight.
Another car is driving on left lane, and it suddenly changes to the lane ego vehicle is driving on.
...
class SuddenLeftLaneChange(BasicScenario):
    """
    ...
    def __init__(self, world, ego_vehicles, config, randomize=False, debug_mode=False, criteria_enable=True,
        ...
        self._velocity = 80  # init velocity of front vehicle
        self._trigger_distance = 20  # decide when the front vehicle change lane

        # get direction from config name
        self._config = config

        super(SuddenLeftLaneChange, self).__init__("SuddenLeftLaneChange",
                                                   ego_vehicles,
                                                   config,
                                                   world,
                                                   debug_mode,
    ...
    def _create_behavior(self):
        ...
        # lane_change
        lane_change = LaneChange(self.other_actors[0], direction='right')
